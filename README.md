# ggadjustedforest <img src="man/figures/logo.svg" align="right" height="139" alt="ggadjustedforest hex logo"/>

<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/ggadjustedforest)](https://CRAN.R-project.org/package=ggadjustedforest)
[![Downloads](https://cranlogs.r-pkg.org/badges/ggadjustedforest)](https://CRAN.R-project.org/package=ggadjustedforest)
[![Total Downloads](https://cranlogs.r-pkg.org/badges/grand-total/ggadjustedforest)](https://CRAN.R-project.org/package=ggadjustedforest)
...
When the estimand is the effect of a single exposure variable,
reporting confounder coefficients alongside it can be misleading. These
coefficients are not identified under the causal model and depend on the full causal structure. `ggadjustedforest` makes it easy to present the exposure effect cleanly, before and after adjustment, reducing susceptibility to false misinterpretations of confounder coefficients.
...
The examples below use the `rotterdam` breast cancer dataset from the **survival** package (2,982 patients; Rotterdam tumour bank). The research question is whether hormonal therapy (`hormon`) affects survival, before and after adjusting for age, tumour size, grade, lymph node involvement, and oestrogen receptor level.
